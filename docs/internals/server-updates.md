...
The launcher is the only runtime writer of `service-state.json`. `pivot service install` and
`pivot service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `pivot-cli@<target>` into a unique staging directory.
...
```sh
npx pivot-cli@<version> service update
```
