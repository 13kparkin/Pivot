...
Pivot connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Pivot works with the platforms your team already uses:
...
- Pivot can suggest titles and descriptions based on your commits
...
- When the review finishes, the run panel shows a **Coverage** checklist — which changed files the review covered versus missed — so a silent skip is visible, not hidden
...
1. Install the GitHub CLI on the machine running Pivot:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Pivot and verify GitHub shows as authenticated
...
If both are set, the access token wins. Restart Pivot and verify the connection in **Source
Control settings**.
...
**Git is required** – Pivot uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Pivot (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
