...
Use this when you want to connect to a Pivot server from another device such as a phone, tablet, or separate desktop app.
...
`pivot pair` finds the running server (the shared `~/.t3` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `pivot pair` says so and points you at `npx pivot-cli serve` or `npx pivot-cli connect`.
...
run a headless server from the CLI, or have the desktop app launch Pivot over SSH.
...
backend with the same server-side behavior as `pivot serve --tailscale-serve`, then the server asks
...
Run the server with `pivot serve`.

```bash
npx pivot-cli serve --host "$(tailscale ip -4)"
```

`pivot serve` starts the server without opening a browser and prints:
...
```bash
npx pivot-cli serve --tailscale-serve
```
...
```bash
npx pivot-cli serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse Pivot on another machine over SSH.
...
During SSH launch, Pivot first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Pivot uses:
...
When the Pivot web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Pivot may
...
see [Keeping Pivot in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Pivot in the Background](./background-service.md).
...
1. `pivot serve` issues a one-time owner pairing token.
...
Use `pivot auth` to manage access after the initial pairing flow.
