# Keeping Pivot in Sync

The Pivot web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Pivot shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Pivot Linux background service. Select the button and leave Pivot open while it prepares, tests, restarts, and reconnects.                              |
| **Update the desktop app** | Open the Pivot desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Pivot server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Pivot does not update connected
servers silently in the background.

An older background-service launcher may ask you to run the exact
`npx pivot-cli@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx pivot-cli@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the Pivot background service, update the service on the host and
pin the same version:

```sh
npx pivot-cli@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx pivot-cli@latest service update`
...
See [Running Pivot in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx pivot-cli@<client-version>`, replacing
