...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "MimeType=x-scheme-handler/pivot;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "pivot",
      desktopEntryPath: "/home/alice/.local/share/applications/pivot-url-handler.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the pivot:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/pivot-url-handler.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the pivot:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/pivot;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "pivot-url-handler.desktop", "x-scheme-handler/pivot"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/pivot-url-handler.desktop",
        }),
      });
