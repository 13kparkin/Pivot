...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["pivot", "connect"]);
  ...
  it.effect("exposes service lifecycle commands without T3 Connect configuration", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Manage the Pivot background service.");
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Next: Run `pivot connect link` to authorize and enable T3 Connect.");
    }),
  );
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["pivot", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["pivot", "project", "add"]);
