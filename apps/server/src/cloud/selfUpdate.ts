...
>()("pivot-cli/cloud/selfUpdate/ServerSelfUpdate") {}
  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the Pivot desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
        "Remote updates require the Pivot background service. Run `pivot service install` on the server machine.",
      );
    }
      ...
      const paths = yield* ensurePinnedRuntimeInstalled({
        ...
        Effect.mapError((error) =>
          error._tag === "PinnedRuntimePreflightBlockedError"
            ...
            : failWith(`Could not prepare pivot-cli@${targetVersion}.`, error),
        ),
      );
