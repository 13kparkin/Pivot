...
describe("ReviewReactor", () => {
  ...
  it("fixes a finding via a dedicated fix session and records the fixed state", async () => {
    const h = await createHarness();
    const reviewId = ReviewId.make("9b1e2f3a-4c5d-4e6f-8a7b-0c1d2e3f4a5b");
    ...
    h.provider.emit({
      type: "turn.completed",
      eventId: "evt-f2" as never,
      provider: ProviderDriverKind.make("omp"),
      threadId: ThreadId.make(`review-${reviewId}`),
      createdAt: NOW,
      turnId: TurnId.make("turn-1"),
      payload: { state: "completed", verdict: "approve", summary: "Clean." },
    });
    await waitForCondition(async () => {
      const runs = await h.readReviewRuns();
      return runs.some((run) => run.id === reviewId && run.status === "completed");
    ...
    await runtime!.runPromise(
      h.engine.dispatch({
        ...
        commandId: CommandId.make("cmd-review-fix"),
        reviewId,
        findingId: "finding-1",
        createdAt: NOW,
      }),
    );
    await waitForCondition(async () => h.provider.sendTurns.length === 2, "fix sendTurn issued");
    const fixTurn = h.provider.sendTurns[1];
    const fixSessionThreadId = h.provider.startSessions[1]?.threadId;
    expect(fixSessionThreadId).toBeDefined();
    expect(fixSessionThreadId?.startsWith("review-fix-")).toBe(true);
    expect(fixTurn?.interactionMode).toBe("default");
    expect(fixTurn?.input ?? "").toContain("Example finding.");

    // The finding shows fixing while the fix session runs.
    await waitForCondition(async () => {
      const runs = await h.readReviewRuns();
      return runs.find((run) => run.id === reviewId)?.findings[0]?.fixState === "fixing";
    }, "finding marked fixing");
    expect(h.provider.startSessions[1]?.cwd).toBe(h.cwd);

    h.provider.emit({
      type: "turn.completed",
      eventId: "evt-f3" as never,
      provider: ProviderDriverKind.make("omp"),
      threadId: fixSessionThreadId,
      createdAt: NOW,
    ...
    await waitForCondition(async () => {
      const runs = await h.readReviewRuns();
      return runs.find((run) => run.id === reviewId)?.findings[0]?.fixState === "fixed";
    }, "finding marked fixed");
    expect(h.provider.stoppedSessions).toContain(fixSessionThreadId);
  });
