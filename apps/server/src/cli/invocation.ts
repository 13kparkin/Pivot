 ...
 * Detection is best-effort; callers must fail closed to a plain bin command.
 */
...
/**
 * The published package spec to suggest. The literal spec the user typed (e.g.
 * `pivot-cli@nightly`) is resolved away before our process starts, so re-derive
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? cliPackageSpec("nightly") : cliPackageSpec();
}

/**
 * Render a `<bin> <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx pivot-cli connect` suggests
 * `npx pivot-cli serve`, a global install suggests `pivot serve`, and a nightly
