...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `pivot connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `pivot connect link` to enable T3 Connect."
      : !status.linked
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `pivot connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of T3 Connect locally.\nThe background service is managed separately with `pivot service`.",
    );
  }
});
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `pivot connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `pivot connect link` before the
  ...
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          yield* Console.log(
            "\n✓ Background service ready\n\nPivot will stay reachable after you log out.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
