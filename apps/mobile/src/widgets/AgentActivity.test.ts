...
describe("AgentActivity widget layout", () => {
  ...
  it("deep links the banner to the row that needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain('"widgetURL":"pivot://threads/env-1/thread-2"');
  });
  ...
  it("deep links the banner to the first row when nothing needs attention", () => {
    ...
    expect(JSON.stringify(layout.banner)).toContain('"widgetURL":"pivot://threads/env-1/thread-1"');
  });
