...
export function AgentActivity(
  ...
  const deepLink =
    deepLinkRow && deepLinkRow.deepLink.startsWith("/") && !deepLinkRow.deepLink.startsWith("//")
      ? `pivot://${deepLinkRow.deepLink.slice(1)}`
      : null;
  ...
  // The branded π mark. `assetName` resolves the template image set bundled in
  ...
  const renderLogo = (height: number, color: string) => (
    <HStack modifiers={[frame({ width: height * 1.297, height }), foregroundStyle(color)]}>
